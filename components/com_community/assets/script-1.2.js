...
joms.extend({
    ...
    sharebox: {
        ...
        uploadVideo: function(creatortype, groupid) {
            ...
            cWindowShow(ajaxCall, '', 450, 500);
        }
    },
    activities: {
        infiniteScrollLoaded: 0,
        infinitesScrollenable: 1,
        init: function() {
            // Capture activity delete button
            joms.jQuery('#activity-stream-container').on( 'click', '.dropdown-menu [data-action=delete]', function( e ) {
                e.preventDefault();
                var li = joms.jQuery( e.target ).closest('[data-streamid]');
            ...
            if ( joms.activities.infinitesScrollenable && !joms.activities.infiniteScrollLoaded ) {
                joms.activities.infiniteScroll();
            }
        },
        showMap: function(id, addr) {
            ...
            if (joms.jQuery('li[data-streamid=' + id + ']').find('.cMapHeatzone').length == 0)
            {
                ...
                mapHTML += '<img src="//maps.google.com/maps/api/staticmap?center=' + addr + '&zoom=14&size=' + mapWidth + 'x150&sensor=false&markers=color:red|' + addr + '" />';
                mapHTML += '<img src="//maps.google.com/maps/api/staticmap?center=' + addr + '&zoom=5&size=' + mapWidth + 'x150&sensor=false&markers=color:red|' + addr + '" />';
                mapHTML += '<img src="//maps.google.com/maps/api/staticmap?center=' + addr + '&zoom=2&size=' + mapWidth + 'x150&sensor=false&markers=color:red|' + addr + '" />';
                mapHTML += '</div>';
            ...
            if (joms.jQuery('#video-' + id).length) {
                ...
                var mapHTML = '<img src="//maps.google.com/maps/api/staticmap?center=' + addr + '&zoom=5&size=' + mapWidth + 'x250&sensor=false&markers=color:red|' + addr + '" />';
        ...
        more: function() {
            // List object
            var stream = joms.jQuery('.cMain > #activity-stream-container ul.list-unstyled');

            //Get stream of group & event
            if (stream.length <= 0)
                stream = joms.jQuery('#activity-stream-container ul.list-unstyled');
            ...
            // Show loading image
            joms.jQuery('#activity-more .btn').hide();
            joms.jQuery('#activity-more .loading').show();
            jax.call('community', 'activities,ajaxGetOlderActivities', exclusions, filter, filterId, stream.data('filter-value'));
        },
        ...
        prependOldStream: function(html)
        {
            ...
            // var btn = container.find('#activity-more');
            // btn.prev('ul').append( html );
            joms.jQuery( html ).children('li').appendTo('.cMain #activity-stream-container > ul.list-unstyled');
            container.find('#activity-more .loading').hide();

            // Show "more" button or clear scroll delay.
            if ( !joms.activities.infiniteScrollLoaded ) {
                joms.jQuery('#activity-more .btn').show();
            } else {
                setTimeout(function() {
                    joms.activities.infiniteScrollDelay = 0;
                }, 1000 );
            }
        },
        /**
        ...
        loadNewStream: function()
        {
            ...
            var stream = joms.jQuery('#activity-stream-container ul');
            ...
            var i = 0
            joms.jQuery('#activity-stream-container > ul li').each(function(){

                if( joms.jQuery(this).data('streamid') ){
        ...
        announceNewStream: function(count, nextPing, textMessage)
        {
            ...
            if (count > 0)
            {
                joms.jQuery('.joms-latest-activities-container')
                        .html('<div class="joms-load-latest"><a href="javascript:" onclick="joms.activities.showNewStream();">' + textMessage + '</div>')
                        .show();
            }
            else
            {
        ...
        {
            // Hide the announcement
            joms.jQuery('.joms-latest-activities-container').hide().empty();
            // Prepend data.
            joms.jQuery( html ).children('li').prependTo('.cMain #activity-stream-container > ul.list-unstyled');
        },
        // User wanted to see the rest of the new stream items
        showNewStream: function()
        {
            var stream = joms.jQuery('#activity-stream-container ul.list-unstyled');
            jax.call('community', 'activities,ajaxGetRecentActivities',stream.find('li[data-streamid]').first().data('streamid'),stream.data('filter'),stream.data('filterid'),stream.data('filter-value'));
            return false;
        },
        ...
        editLocation: function(activityid) {
           var ajaxCall = "jax.call('community', 'activities,ajaxeditLocation', '" + activityid + "');";
           cWindowShow(ajaxCall, '', 450, 100);
        },
        saveLocation: function(activityid,elem){
            var ct = joms.jQuery('#cWindow').find('#editlocation'),
                input = ct.find('#editlocation-selector input'),
                map = ct.find('#editlocation-map'),
                name = input.val(),
                lat = map.data('lat'),
                lng = map.data('lng');
            ...
            jax.call( 'community','activities,ajaxSaveLocation', activityid, name, lat, lng );
        },
        removeLocation: function(activityid){
            var ajaxCall = "jax.call('community', 'activities,ajaxRemoveLocation', '" + activityid + "');";
           cWindowShow(ajaxCall, '', 450, 100);
        },
        infiniteScroll: function () {
            ...
            var mobile = !!ua.match( /android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini/i );
            if ( mobile )
                return;

            // Cache window and document for faster access.
            var win = joms.jQuery( window ),
                doc = joms.jQuery( document );
            ...
            win.on( 'scroll', function() {
                if ( win.scrollTop() == doc.height() - win.height() ) {
                    if ( joms.activities.infiniteScrollDelay )
                        return;

                    joms.activities.infiniteScrollDelay = 1;
                    joms.activities.more();

                    // reset delay flag in case of request timeout
                    setTimeout(function() {
                        joms.activities.infiniteScrollDelay = 0;
                    }, 10000 );
                }
            ...
            // Update infinite scroll flag.
            joms.activities.infiniteScrollLoaded = 1;
        }
    },
    apps: {
        windowTitle: '',
    ...
    invitation: {
        ...
        send: function(callback, cid) {
            var form = joms.jQuery('#community-invitation-form'),
                values = form.serializeArray(),
                friends = [],
                params = [];

            if ( values && values.length ) {
                for ( var i = 0, val; i < values.length; i++ ) {
                    val = values[i];
                    if ( val.name === 'friends[]' && friends.indexOf( val.value ) < 0 )
                        friends.push( val.value );
                }
            }

            params.push([ 'friendsearch', form.find('[name=friendsearch]').val() ]);
            params.push([ 'emails', form.find('[name=emails]').val() ]);
            params.push([ 'message', form.find('[name=message]').val() ]);
            friends.length && params.push([ 'friends', friends.join(',') ]);

            jax.call('community', 'system,ajaxSubmitInvitation', callback, cid, params);
        },
    ...
    photos: {
        multiUpload: {
                ...
                pluploader = joms.jQuery("#multi_uploader").pluploadQueue({
                    ...
                    preinit: {
                        ...
                        BeforeUpload: function() {
                            if (joms.jQuery("#new-album").css('display') !== 'none')
                            {
                        ...
                        FileUploaded: function(uploader, file, response) {
                            ...
                            if (uploadData != undefined && uploadedFile != undefined)
                            {
                                if (uploadData.error != undefined && uploadData.photoId == '') // there is error
                                {
                                    ...
                                    joms.jQuery('.plupload_file_name span').bind('click', function(event) {
                                        joms.photos.multiUpload.editTitle(event)
                                    });
                                }
                                else // update photo details into file dictionary
                                {
                                    joms.jQuery('#album-name').attr('albumid', uploadData.albumId);
            ...
            showThumbnail: function() {
                ...
                for (var i = 0; i < uploader.files.length; i++)
                {
                    ...
                    joms.jQuery('#' + uploader.files[i].id + '.plupload_done .plupload_file_name').prepend(imgNode);
                }
            ...
            uploadCompleteShow: function() {
                ...
                cWindowResize();
                var uploader = pluploader.pluploadQueue();
                joms.ajax.call('photos,ajaxUpdateCounter', [joms.photos.multiUpload.getSelectedAlbumId(),JSON.stringify(uploader)]);
            },
        ...
        photoSlider: {
            ...
            _init: function(elementId, stopAnimeId, options) {
                ...
                joms.photos.photoSlider.parentElem.bind({
                    ...
                    'mousemove': function(event) {
                        joms.photos.photoSlider.updateMousePos(event);
                    },
                    'mouseout': function(event) {
                        joms.photos.photoSlider.reset(event);
                    }
                });
            ...
            switchPhoto: function() {
                ...
                if ( hash && hash.match( /^#photoid=\d+$/ ) ) {
                    currentPhotoId = hash.replace( '#photoid=', '' );
                }
                if ( !currentPhotoId ) {
                    currentPhotoId = href.match(/.+photoid=(\d+).*/);
                    currentPhotoId = currentPhotoId && currentPhotoId[1];
                }
                if ( !currentPhotoId ) {
                    currentPhotoId = joms.photos.photoSlider.controlObj.find(img).eq(0).attr('id');
                }
                ...
                var img = joms.jQuery('img#photoSlider_thumb' + currentPhotoId).css('opacity', joms.photos.photoSlider.fullOpacity);
                joms.jQuery('.image_thumb').not(img).css('opacity', joms.photos.photoSlider.partialOpacity);

                // Change data-src to src
                if ( !img.attr('src') && img.data('src') )
                    img[0].src = img.data('src');

                // Change next data-src to src
                var next = img;
                for ( var i = 0; i < 6; i++ ) {
                    next = next.closest('li').next('li');
                ...
                // Change prev data-src to src
                var prev = img;
                for ( var i = 6; i > 0; i-- ) {
                    prev = prev.closest('li').prev('li');
                    if ( !prev.length ) break;
                    prev = prev.find('img');
                    if ( prev.length && !prev.attr('src') && prev.data('src') )
                        prev[0].src = prev.data('src');
                }
                ...
                var li = img.closest('li'),
                    ...
                    div = ul.closest('div'),
                    divWidth = div.width(),
                    divHalf = divWidth / 2 - li.outerWidth( true ) / 2;

                if ( ulWidth < divWidth )
                    ulLeft = divWidth / 2 - ulWidth / 2;
                else
                    ulLeft = Math.max( 0 - (ulWidth - divWidth), Math.min( 0, 0 - liLeft + divHalf ) );

                ul.stop().animate({ left: ulLeft });
            },
    ...
    walls: {
        ...
        insert: function(html) {
            ...
            if (joms.walls.insertOrder == 'prepend') {
                ...
                if(joms.jQuery('#wall-containter .cComment:last').length > 0){
                    joms.jQuery('#wall-containter .cComment:last').after(html);
                }else{
                    joms.jQuery('#wall-containter').prepend(html);
                }
            }
        ...
        remove: function(type, wallId, contentId) {
            if (confirm('Are you sure you want to delete this wall?'))
            {
                ...
                joms.jQuery('#wall_' + wallId).fadeOut('normal', function() {
                    joms.jQuery(this).remove()
                });
    ...
    miniwall: {
        initialize: function() {
            var container = joms.jQuery('#activity-stream-container');

            // Support comment removal
            joms.jQuery('a[href="#removeComment"]').on( 'click', function() {
                var wallid = joms.jQuery(this).parents('[data-commentid]').data('commentid');
                joms.miniwall.remove(wallid);
            });

            // Show all comments.
            container.on( 'click', 'a[href="#showallcomments"]', function( e ) {
                e.preventDefault();
                ...
                el.data( 'loading', 1 );
                var streamid = el.closest('li').data('streamid');
        ...
        add: function(id, host) {
            ...
            var textarea = cmtBlock.find('textarea');

            if ( !textarea.data('inittextntags') ) {
                joms.miniwall._add( id, cmtBlock, textarea.val() );
                return;
            }

            textarea.textntags( 'val', function( cmt ) {
                joms.miniwall._add( id, cmtBlock, cmt );
            });
        },
        _add: function( id, cmtBlock, cmt ) {
            ...
            if (cmt.length > 0)
            {
                ...
                jax.loadingFunction = function() {
                    ...
                    jax.loadingFunction = function() {
                    }             // clear the callbacks after each call
                };
    ...
    comments: {
        ...
        saveEdit: function(id, parentid, host) {
            ...
            if ( !textarea.data('inittextntags') ) {
                jax.call('community', 'system,ajaxeditComment', id, textarea.val(), parentid);
                return;
            }

            textarea.textntags( 'val', function( cmt ) {
                jax.call('community', 'system,ajaxeditComment', id, cmt, parentid);
            });
        }
    },
    utils: {
    ...
    // asynchronous google map loader
    map: {
    ...
    connect: {
        checkRealname: function(value) {
            ...
            jax.doneLoadingFunction = function() {
                jax.loadingFunction = tmpLoadingFunction;
            };
        ...
        validateNewAccount: function() {
            ...
            if(joms.jQuery('.jsProfileType').length > 0 && joms.jQuery('.jsProfileType input:checked').length == 0){
               jax.call('community', 'connect,ajaxCheckProfileType');
               isValid = false;
            }
    ...
    users: {
        ...
        /**
        * Do display block confirm dialog
        */
        confirmBlockUser: function(userId) {
            var ajaxCall = 'jax.call("community", "profile,ajaxConfirmBlockUser", "' + userId + '");';
            cWindowShow(ajaxCall, '', 450, 100);
        },
        /**
         * Do block user
        ...
        /**
        * Do display unblock confirm dialog
        */
        confirmUnBlockUser: function(userId) {
            var ajaxCall = 'jax.call("community", "profile,ajaxConfirmUnBlockUser", "' + userId + '");';
            cWindowShow(ajaxCall, '', 450, 100);
        },
        /**
         * Do unblock user
        ...
        /**
         * Do display ignore confirm dialog
         */
        confirmIgnoreUser: function(userId) {
            var ajaxCall = 'jax.call("community", "profile,ajaxConfirmIgnoreUser", "' + userId + '");';
            cWindowShow(ajaxCall, '', 450, 100);
        },
        /**
        * Do ignore
        */
        ignoreUser: function(userId) {
            var ajaxCall = 'jax.call("community", "profile,ajaxIgnoreUser", "' + userId + '");';
            cWindowShow(ajaxCall, '', 450, 100);
        },
         /**
         * Do display Unignore confirm dialog
         */
        confirmUnIgnoreUser: function(userId) {
            var ajaxCall = 'jax.call("community", "profile,ajaxConfirmUnIgnoreUser", "' + userId + '");';
            cWindowShow(ajaxCall, '', 450, 100);
        },
        /**
         * Do unignore user
        ...
        unIgnoreUser: function(userId) {
            var ajaxCall = 'jax.call("community", "profile,ajaxUnIgnoreUser", "' + userId + '");';
            cWindowShow(ajaxCall, '', 450, 100);
        },
    ...
    events: {
        ...
        submitRSVP: function(eventId, e) {
            var select = joms.jQuery( e ),
                parent = select.parent(),
                label = select.prev('span'),
                value = select.val(),
                klass = select.attr('class'),
                text = select.find('option:selected').text();

            if ( joms.events.submitRSVP.processing )
                return false;

            // create unclickable layer
            ...
            setTimeout(function() {
                ...
                joms.ajax.call( 'events,ajaxUpdateStatus', [ eventId, select.val() ], {
                    success: function( html ) {
                        joms.events.submitRSVP.processing = false;
                        joms.jQuery("#community-event-members").replaceWith( html );
                        label.attr('class', select.find('option:selected').attr('class') );
                        label.html( text );
                        parent.children('.noclick').remove();
                    }
                });
            }, 100 );
        },
    ...
    file: {
        ...
        ajaxUploadFile: function(url, type, size)
        {
            ...
            var pluploader = joms.file.___uploader = joms.jQuery("#html5_uploader").pluploadQueue({
                // General settings
            ...
            // fix the button for html4
            if ( pluploader.pluploadQueue().runtime == 'html4' ) {
                joms.file.ieUploadButtonFix();
            }
        ...
        ieUploadButtonFix: function() {
            var uploader = joms.file.___uploader;
            if ( !uploader ) return;
            var btn  = uploader.find('.plupload_add');
            var form = uploader.find('form');
            var file = form.find('input[type=file]');
            if ( !btn.length || !file.length ) return;
            btn.wrap('<span/>');
            var span = btn.parent();
            span.css({
        ...
        startdownload: function(url)
        {
            ...
            url = url.replace('&amp;', '&');
            joms.jQuery('<iframe>', {src: url, id: id}).appendTo('body');
            //cWindowHide();
    ...
    cover: {
                    ...
                    (
                            {
                                ...
                                success: function(data, status)
                                {
                                    ...
                                    if (data.error)
                                    {
                                        ...
                                        joms.jQuery('p#error_msg').fadeOut(7000);
                                    }
                                }
                            }
                    )
        ...
        reposition: function(e, id, cancelText, SaveText) {
            var changeCoverButtons = joms.jQuery('.js-focus-change-cover');
            changeCoverButtons.data('display', changeCoverButtons[0].style.display);
            changeCoverButtons.hide();
            ...
            var button = '<button class="btn btn-primary" onClick="joms.cover.repositionCancel(' + id + ',\'' + currentTop + '\')"><span>' + cancelText + '</span></button><button class="btn btn-primary ml-6" onClick="joms.cover.saveReposition()"><span>' + SaveText + '</span></button>';
            ...
            joms.jQuery('img#' + id).draggable({
                ...
                stop: function(event, ui) {
                    var img = joms.jQuery('img#' + id),
                        cover = img.closest('.js-focus-cover'),
                        percent = ui.position.top * 100 / cover.height();

                    // 4-digit precision
                    percent = Math.round( percent * 10000 ) / 10000;

                    joms.cover.position = ui.position.top;
                    joms.cover.positionPercent = percent;
                    img.css('top', percent + '%');
                }
            });
        },
        repositionCancel: function(id, currentTop) {
            var changeCoverButtons = joms.jQuery('.js-focus-change-cover');
            changeCoverButtons.css({ display: changeCoverButtons.data('display') });
        ...
        {
            jax.call("community", "photos,ajaxSetPhotoPhosition", joms.cover.type, joms.cover.parentId, joms.cover.positionPercent + '%');
            ...
            joms.jQuery('.js-focus-change-cover a.btn.hidden').removeClass('hidden');
        }
    },
    share: {
        popup: function(activityId) {
            joms.jQuery(document).off("click","#cWindowAction button.btn-primary");
            var ajaxCall = "jax.call('community', 'activities,ajaxSharePopup','" + activityId + "');";
            cWindowShow(ajaxCall, '', 450, 300);
        },
        add: function(activityId, data) {
            joms.jQuery(document).off("click","#cWindowAction button.btn-primary");
            jax.call('community', 'activities,ajaxAddShare', activityId, data);
        },
        map: function(activityId) {
            var ajaxCall = "jax.call('community', 'activities,ajaxShowMap','" + activityId + "');";
            cWindowShow(ajaxCall, '', 650, 300);
        }
    }
});
...
joms.jQuery(document).ready(function() {
    ...
    // Initialize stream
    if (joms.jQuery('.list-unstyled li').length) {
        joms.miniwall.initialize();
    }
...
(function(f) {
    ...
    f(document).ready(function() {
        ...
        var s;
        f(window).bind("scroll", function(t) {
            clearTimeout(s);
            s = setTimeout(function() {
                ...
                for (i = 0; i < f.fn.qtip.interfaces.length; i++) {
                    ...
                    if (u.status.rendered === true && (u.options.position.type !== "static" || u.options.position.adjust.scroll && t.type === "scroll")) {
