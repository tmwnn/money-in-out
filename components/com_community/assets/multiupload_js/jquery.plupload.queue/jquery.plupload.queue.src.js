...
(function($) {
	...
	$.extend(languages, joms && joms.language && joms.language.multiupload || {});
			...
			this.each(function() {
				...
				function updateList() {
					...
					$.each(uploader.files, function(i, file) {
						...
						fileList.append(
							'<li id="' + file.id + '">' +
								...
								'<div class="plupload_file_size">' + languages.size + ': ' + plupload.formatSize(file.size) + '</div>' +
								'<div class="plupload_file_status">' + languages.status + ': ' + file.percent + '%</div>' +
								inputHTML +
							'</li>'
						);
					...
					if (!uploader.files.length && uploader.features.dragdrop && uploader.settings.dragdrop) {
						$('#' + id + '_filelist').append('<li class="plupload_droptext">' + languages.drag_files + '</li>');
					}
				}
				...
				uploader.bind('Init', function(up, res) {
					...
					// Bugfix open file button on IE 10, and Opera 12 and lower (Presto engine).
					...
					if (window.opera) {
						...
						ie = !!ua.match(/msie/i);
						version = ie && +( ua.match(/msie (\d+)\./i)[1] );
						if (ie && version === 10)
							manual = true;
					}

					if (manual) {
						$('#' + id + '_browse').click(function() {
							$('#' + id + ' .plupload input[type=file]').click();
						});
					}
