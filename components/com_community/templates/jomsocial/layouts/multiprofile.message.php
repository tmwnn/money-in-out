...
<?php if( $multiprofile->approvals && !$isCommunityAdmin ){
	CSystemHelper::showNoticePage(JText::sprintf('COM_COMMUNITY_PROFILE_CHANGE_REQUIRE_APPROVALS_INFO' , $multiprofile->name ),'COM_COMMUNITY_PROFILE_CHANGE_INFO_MSG');
	?>
	<!--
	...
	-->
<?php } else {
	CSystemHelper::showNoticePage(JText::sprintf('COM_COMMUNITY_PROFILE_CHANGE_INFO' , $multiprofile->name ),'COM_COMMUNITY_PROFILE_CHANGE_INFO_MSG', array(CRoute::_('index.php?option=com_community&view=profile')=>JText::_('COM_COMMUNITY_RETURN_TO_PROFILE')));
	?>
