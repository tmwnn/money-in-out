...
function modChrome_gk_style($module, $params, $attribs) {	
	...
	if (!empty ($module->content)) {		
		...
		if(stripos($params->get('moduleclass_sfx'), 'bigtitle') !== FALSE) {
			$header_type = '2';
		}
		
		echo '<div class="box ' . $params->get('moduleclass_sfx') . ($attribs['name'] == 'header_bottom' ? ' color ' : '') . $modnum_class . $overflow_class . $margin_class . '">';
		
		if($module->showtitle) {	
			if($params->get('module_link_switch')) {
				echo '<h'.$header_type.' class="header"><a href="'. $params->get('module_link') .'">'. preg_replace(array('/__(.*?)__/i', '/\[br\]/i'), array('<small>${1}</small>', '<br />'), $module->title) .'</a></h'.$header_type.'>';
			} else {
				echo '<h'.$header_type.' class="header">'. preg_replace(array('/__(.*?)__/i'), array('<small>${1}</small>'), $module->title) .'</h'.$header_type.'>';
			}
		}
